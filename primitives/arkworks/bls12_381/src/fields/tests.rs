...
fn test_fq_repr_div2() {
	...
	assert_eq!(a, BigInt::new([0x7288af1f36ee3608, 0x1e8, 0x0, 0x0, 0x0, 0x0]));
	for _ in 0..50 {
		a.div2();
	}
...
fn test_fq_repr_divn() {
	...
	assert_eq!(
		a,
		BigInt::new([0xa0fea40235bf3cee, 0x4067a038f0582e2a, 0x2f9480d7528a70b0, 0x91, 0x0, 0x0,])
	);
	a.divn(64);
	assert_eq!(a, BigInt::new([0x4067a038f0582e2a, 0x2f9480d7528a70b0, 0x91, 0x0, 0x0, 0x0]));
}
...
fn test_fq_repr_mul2() {
	...
	assert_eq!(a, BigInt::new([0x6000000000000000, 0xb0acd6c9, 0x0, 0x0, 0x0, 0x0]));
	for _ in 0..300 {
		a.mul2();
	}
	assert_eq!(a, BigInt::new([0x0, 0x0, 0x0, 0x0, 0x0, 0xcd6c960000000000]));
	for _ in 0..17 {
		a.mul2();
	}
	assert_eq!(a, BigInt::new([0x0, 0x0, 0x0, 0x0, 0x0, 0x2c00000000000000]));
	for _ in 0..6 {
		a.mul2();
	}
	assert!(a.is_zero());
...
fn test_fq_root_of_unity() {
	...
	assert_eq!(Fq::TWO_ADIC_ROOT_OF_UNITY.pow([1 << Fq::TWO_ADICITY]), Fq::one());
...
fn test_fq_legendre() {
	...
	assert_eq!(QuadraticNonResidue, Fq::from(BigInteger384::from(2u64)).legendre());
	assert_eq!(QuadraticResidue, Fq::from(BigInteger384::from(4u64)).legendre());
...
fn test_fq2_squaring() {
	...
	assert_eq!(a, Fq2::new(Fq::zero(), Fq::from(BigInteger384::from(2u64)),)); // 2u
